...
// ── AI engine ──
const SAFETY = `You are Hakim, a compassionate AI Healthcare Assistant for Canadian residents. You provide educational health guidance only. You never diagnose. You always recommend professional care. You are warm, specific, and clear. Never use em dashes. For emergencies (chest pain, stroke, overdose, suicidal ideation) immediately direct to 911 or 811.`;
...
function SetupView({patient,onSave,lang}){
  ...
  return<div style={{padding:"24px 16px",maxWidth:440,margin:"0 auto"}}>
    <div style={{fontSize:10,letterSpacing:2,textTransform:"uppercase",color:C.TEAL,marginBottom:4}}>Hakim · ሐኪም</div>
    <h2 style={{fontSize:22,fontWeight:700,color:C.PAPER,margin:"0 0 4px"}}>Your Health Profile</h2>
    <p style={{fontSize:13,color:C.DIM,marginBottom:20,lineHeight:1.5}}>Stays on your device only — never shared. Powers your body map, lab scores, and screening schedule.</p>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"12px 14px 20px"}}>
        ...
        {tab==="eval"&&<div style={{maxWidth:600}}>
          {!ev&&!evalBusy&&<div style={{textAlign:"center",padding:"24px 0"}}>
            <p style={{color:C.DIM,fontSize:13,lineHeight:1.55,marginBottom:14}}>Hakim will read your {am?sys.am:sys.name} labs and give you a personalised explanation.</p>
        ...
        {tab==="ask"&&<div style={{display:"flex",flexDirection:"column",height:"100%"}}>
          <div ref={askRef} style={{flex:1,overflowY:"auto",minHeight:120,marginBottom:10}}>
            {(!askThread[key]||askThread[key].length===0)&&<p style={{color:C.DIM,fontSize:12,lineHeight:1.5}}>Ask anything about your {am?sys.am:sys.name} system. Hakim will answer with your data in view.</p>}
            {(askThread[key]||[]).map((m,i)=><div key={i} style={{marginBottom:10,display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start"}}>
              <div style={{maxWidth:"90%",background:m.role==="user"?C.PANEL3:"transparent",border:m.role==="dr"?`1px solid ${C.FAINT}`:"none",borderLeft:m.role==="dr"?`3px solid ${sys.accent}`:"none",borderRadius:m.role==="user"?10:4,padding:"8px 11px",fontSize:12,lineHeight:1.55,whiteSpace:"pre-wrap"}}>
                {m.role==="dr"&&<div style={{fontSize:9,letterSpacing:1.5,textTransform:"uppercase",color:sys.accent,marginBottom:4}}>Hakim</div>}
                {m.text}
              </div>
            </div>)}
...
function EvalView({d,accent,lang,onRerun}){
  ...
  return<div>
    ...
    {Array.isArray(d.findings)&&d.findings.length>0&&<S title="What Hakim noticed">
...
function ScreeningsView({patient,lang,apiKey,onToggleDone}){
  ...
  return<div style={{padding:"12px 14px",overflowY:"auto",height:"100%"}}>
    ...
    {!advice&&!adviceBusy&&<button onClick={getAdvice} style={{width:"100%",padding:"10px",background:C.TEAL,border:"none",borderRadius:9,color:C.INK,fontFamily:C.SANS,fontSize:13,fontWeight:600,cursor:"pointer"}}>
      {am?"ምክር ያግኙ":"Get personalised advice from Hakim"}
    </button>}
    ...
    {advice&&<div style={{marginTop:8,background:C.PANEL2,borderLeft:`3px solid ${C.TEAL}`,padding:"12px 14px",fontSize:12,lineHeight:1.65,whiteSpace:"pre-wrap",borderRadius:4}}>
      <div style={{fontSize:9,letterSpacing:2,textTransform:"uppercase",color:C.TEAL,marginBottom:6}}>Hakim</div>
      {advice}
  ...
  return<div style={{display:"flex",flexDirection:"column",height:"100%",overflow:"hidden"}}>
    ...
    <div style={{flex:1,overflowY:"auto",padding:"14px"}}>
      ...
      {wview==="synthesis"&&<div style={{maxWidth:600}}>
        <p style={{fontSize:12,color:C.DIM,marginBottom:14,lineHeight:1.5}}>{am?"ሁሉንም ሥርዓቶች አጣምረህ ትንተና።":"Hakim reads all your systems together and tells the story they tell as a whole."}</p>
      ...
      {wview==="child"&&<div style={{maxWidth:600}}>
        <p style={{fontSize:12,color:C.DIM,marginBottom:10,lineHeight:1.5}}>{am?"ለህጻን ጥንቃቄ የሞላው ምርመራ — ምርመራ አይደለም።":"A guarded developmental screen for children. Hakim never diagnoses a child."}</p>
  ...
  return<div style={{padding:"12px 14px",overflowY:"auto",height:"100%"}}>
    ...
    {report?.data&&<div style={{background:"#fff",color:"#1A2E22",borderRadius:10,padding:"18px",boxShadow:"0 4px 20px rgba(0,0,0,.4)"}}>
      <div style={{borderBottom:"2px solid #1F6E43",paddingBottom:8,marginBottom:12}}>
        <div style={{fontSize:9,letterSpacing:2,color:"#6B8F71",textTransform:"uppercase"}}>Hakim · ሐኪም Health OS</div>
...
// ── CHAT VIEW ── (placeholder; full chat is built into the AI buttons throughout the OS)
function ChatView({apiKey,lang}){
  const am=lang==="am";
  return<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100%",padding:"24px"}}>
    ...
    <div style={{fontSize:14,fontWeight:600,color:C.PAPER,marginBottom:6}}>{am?"AI ምክር":"AI Guidance"}</div>
    <p style={{fontSize:12,color:C.DIM,textAlign:"center",lineHeight:1.6,marginBottom:16,maxWidth:280}}>
      {am?"AI ምክር ለማግኘት ⚙️ ተጭነው API ቁልፍዎን ያስገቡ። ከዚያ ማንኛውንም ሥርዓት ክፍት አድርገው 'Evaluate' ወይም 'Ask' ይምረጡ።"
        :"Use the Systems, Wellness, Screenings and Reports tabs to get AI-powered guidance. Press ⚙️ in the header to connect your API key."}
    </p>
    {!apiKey&&<div style={{background:"rgba(245,158,11,.1)",border:"1px solid rgba(245,158,11,.3)",borderRadius:9,padding:"10px 14px",fontSize:11,color:"#FCD34D",textAlign:"center"}}>
      {am?"API ቁልፍ አልተገናኘም — ⚙️ ተጭነው ያስገቡ":"No API key connected — tap ⚙️ in the header to add one"}
    </div>}
  </div>;
}

// ── MAIN DrRebira component ──
export default function HakimOS({ lang, apiKey, onEmergency }) {
